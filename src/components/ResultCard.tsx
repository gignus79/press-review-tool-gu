      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
                ...
                {result.isDuplicate && (
                  <>
                    ...
                    <div className="flex items-center gap-1 text-accent">
                      <Copy size={14} />
                      <span className="text-xs font-medium uppercase tracking-wide">Duplicate</span>
                    </div>
                  </>
                )}
              </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {result.isAnalyzing ? (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Sparkle size={16} className="animate-pulse text-secondary" weight="fill" />
                <span>Analyzing...</span>
              </div>
            ) : result.analysis ? (
              <>
                ...
                <Badge variant="secondary">
                  Relevance: {result.analysis.relevanceScore}%
                </Badge>
                <Badge variant="outline">
                  Authority: {result.analysis.authority}/100
                </Badge>
                {result.analysis.themes.slice(0, 2).map((theme) => (
